...
def initsystem(nplates):
    ...
    system = model.System(coor, conn, dofs, iip, stitch.elemmap(), stitch.nodemap(), is_plastic)

    system.setDriveStiffness(1e-3, 1)
    system.setMassMatrix(rho * np.ones((nelem)))
    system.setDampingMatrix(alpha * np.ones((nelem)))
    system.setElastic(K * np.ones((nelem)), G * np.ones((nelem)))
    system.setPlastic(K * np.ones((nelem)), G * np.ones((nelem)), 1e9 * np.ones((nelem, 1)))
    system.setDt(dt)

    return system, drive, Hi
# ---------

system, drive, Hi = initsystem(5)
drive[:, 0] = True
...
for i in range(5):
    system.addAffineSimpleShear(delta_gamma[i], drive, Hi)
    print(system.layerUbar() - system.layerTargetUbar())
