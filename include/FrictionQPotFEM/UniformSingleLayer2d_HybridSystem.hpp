...
#ifndef FRICTIONQPOTFEM_UNIFORMSINGLELAYER2D_HYBRIDSYSTEM_HPP
#define FRICTIONQPOTFEM_UNIFORMSINGLELAYER2D_HYBRIDSYSTEM_HPP

#include "UniformSingleLayer2d_HybridSystem.h"
...
inline void HybridSystem::setU(const xt::xtensor<double, 2>& u)
{
    FRICTIONQPOTFEM_ASSERT(xt::has_shape(u, {m_nnode, m_ndim}));
    m_u = u;
    this->computeStressPlastic();
}

inline void HybridSystem::quench()
{
    m_v.fill(0.0);
    m_a.fill(0.0);
}
...
inline auto HybridSystem::forceMaterial() const
{
    return m_felas + m_fplas;
}

inline double HybridSystem::residual() const
{
...
inline double HybridSystem::t() const
{
    return m_t;
}

inline auto HybridSystem::dV() const
{
    return m_quad.dV();
}

inline auto HybridSystem::u() const
{
    return m_u;
}

template <size_t rank, class T>
inline auto HybridSystem::AsTensor(const T& arg) const
{
