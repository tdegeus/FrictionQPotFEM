...
// #define NORMCLOSE(a, b) a <= 0 && b <= 0 ? REQUIRE_THAT((a), Catch::WithinAbs((b), 1e-8)) : REQUIRE(a == Approx(b));
...
SECTION("System vs. HybridSystem")
{
    ...
    full.setPlastic(K * xt::ones<double>({elastic.size()}), G * xt::ones<double>({elastic.size()}), epsy);
    reduced.setPlastic(K * xt::ones<double>({elastic.size()}), G * xt::ones<double>({elastic.size()}), epsy);
    ...
    for (size_t inc = 0 ; inc < dF.shape(0); ++inc) {
        ...
        for (size_t iiter = 0; iiter < 99999 ; ++iiter) {
            ...
            REQUIRE(xt::allclose(full.forceMaterial(), reduced.forceMaterial()));
            REQUIRE(xt::allclose(full.u(), reduced.u()));
